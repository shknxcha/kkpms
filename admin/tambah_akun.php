<?php 
    ...
    while($anggota = mysqli_fetch_assoc($sql)){
        ...
        $sqlUser = mysqli_query($conn, "SELECT * FROM user WHERE id_anggota = $idAnggota");
        if(!mysqli_fetch_assoc($sqlUser)){
            $dataAnggota[] = $anggota;
        }
    }
    $no = 0;
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Penambahan Data Akun</h6>
    </div>
    <div class="card-body">
    <form method="post" action="tambah_aksi_akun.php"  enctype="multipart/form-data">
    <div class="mb-3">
        <label  class="form-label">Nama Anggota</label>
            <select class="form-select" name="nama_anggota">
                <option selected>--Pilih Anggota--</option>
                <?php for($i=0; $i < count($dataAnggota); $i++) { ?>
                <option value="<?=$dataAnggota[$i]['id_anggota']?>"><?=$dataAnggota[$i]["nama_anggota"]?></option>
                <?php } ?>
            </select>
    </div>
    <div class="mb-3">
    ...
    <div class="form-group mb-3">
        ...
        <input type="password" name="konPassword" class="form-control" placeholder="Konfirmasi Username">
    </div>
